...
class GetDataSet(torch.utils.data.Dataset):
    ...
    def __init__(self, datafile, normalize =False,sample_length =1024,sample_nums=470,train_nums=370,slice_stride=None,data_label =0):
        ...
        self._sample_nums =sample_nums
        self.train_nums =train_nums
        self.__slice_stride=slice_stride
    ...
    def __split_train_test_data(self):#私有方法 按要求截取归一化后的 训练集和测试集 保存到test_data 和 train_data属性中
        ...
        #如果没有输入步长 就按不重叠顺序采样
        if self.__slice_stride is None:
            for i in range(self._sample_nums):#需要截出多少样本循环多少次
                self.slice_length =slice(self.strat_index,(i+1)*self._sample_length)
                ...
                self.strat_index+=self._sample_length#对起始索引累加更新
            ...
            print('test_nums= ',self._sample_nums -self.train_nums)


        #如果输入了步长，就按按步长顺序重叠采样
        else:
            for i in range(self._sample_nums):
                self.slice_length =slice(self.strat_index,self.strat_index+self._sample_length)
                self.data =self.dataFrame[:,self.slice_length]
                self.sample_blk.append(self.data)
                self.strat_index+=self.__slice_stride
...
a =GetDataSet('D:\\SPYDER_CODE\\MyProject\\DataSetA\\12k-0.007-1-1772\\X106_DE_time.csv',normalize =True,slice_stride =200)